...
var carBlueStartX =310 ; 
var carBlueStartY = 185; 
...
blueCarSprites.src = "img/biker.png";
...
function draw(){	
	...
	if(isBlueActive == true){
		context.drawImage(blueCarSprites,0,0,carBlue.width,carBlue.height,carBlue.x,carBlue.y,carBlue.width/2,carBlue.height/2);
	}
...
function blueCarMove(){ 
	if(counter < 190){
		...
		if(deltaB > intervalB){
			context.clearRect(0,0,canvas.width,canvas.height);
			draw();
			context.drawImage(blueCarSprites,cycleLoopBlue[currentLoopIndexBlue]*carBlue.width,0,carBlue.width,carBlue.height,carBlue.x,carBlue.y,carBlue.width/2,carBlue.height/2);
			carBlue.newPos(10,0);
